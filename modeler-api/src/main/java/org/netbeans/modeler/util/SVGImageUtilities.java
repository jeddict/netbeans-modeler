...
public class SVGImageUtilities {
...
//        GIF_READER = ImageIO.getImageReadersByMIMEType("image/gif").next();
    ...
    /**
     * Loads an image from the specified resource ID. The image is loaded using
     * the "system" classloader registered in Lookup.
     *
    ...
    /**
     * Loads an image based on resource path. Exactly like
     * {@link #loadImage(String)} but may do a localized search. For example,
     * requesting <samp>org/netbeans/modules/foo/resources/foo.gif</samp>
     * might actually find
    ...
    /**
     * Loads an icon based on resource path. Similar to
     * {@link #loadImage(String, boolean)}, returns ImageIcon instead of Image.
     *
     ...
     * over the first one with its top-left corner at x, y. Images need not be
     * of the same size. New image will have a size of max(second image size +
     * top-left corner, first image size). Method is used mostly when second
     * image contains transparent pixels (e.g. for badging). Method that
     * attempts to find the merged image in the cache first, then creates the
     ...
     * original remains unmodified) Text can contain HTML tags e.g.
     * "&#60;b&#62;my&#60;/b&#62; text"
     *
    ...
    /**
     * Add text to tool tip for given image (creates new or returns cached,
     * original remains unmodified) Text can contain HTML tags e.g.
     * "&#60;b&#62;my&#60;/b&#62; text"
     *
    ...
    /**
     * Creates disabled (color saturation lowered) icon. Icon image conversion
     * is performed lazily.
     *
    ...
    /**
     * Get the class loader from lookup. Since this is done very frequently, it
     * is wasteful to query lookup each time. Instead, remember the last result
     * and just listen for changes.
     */
     ...
     * @param loader classloader to use for locating it, or null to use
     * classpath
...
//                Image img2 = toBufferedImage(result);
    ...
    private static class ToolTipImage extends BufferedImage implements Icon {
        ...
        public static ToolTipImage createNew(String toolTipText, Image image, URL url) {
            SVGImageUtilities.ensureLoaded(image);
